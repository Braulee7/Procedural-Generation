...
namespace evn_util {
	...
	private: // methods
		...
		inline float linear(float start, float end, float coef) { return coef * (end - start) + start; }
		inline float poly(float coef) { return 3 * coef * coef - 2 * coef * coef * coef; }
		inline float interp(float start, float end, float coef) { return linear(start, end, poly(coef)); }
	private:
